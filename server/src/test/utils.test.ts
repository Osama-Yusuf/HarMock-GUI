...
describe('relaxedQuery', () => {
    it('drops volatile keys', () => {
        const q = relaxedQuery({ a: ['1'], _t: ['9'], cache: ['y'] } as any);
...
describe('chooseBestMatch', () => {
    it('ranks by specificity then orderIdx', () => {
        const e = chooseBestMatch([
            { tier: 'PathOnly', entry: { orderIdx: 2 } as any },
            { tier: 'Exact', entry: { orderIdx: 5 } as any },
            { tier: 'Relaxed', entry: { orderIdx: 1 } as any }
        ]);
        expect((e as any)?.orderIdx).toBe(5);
    });
});
